using ProjectJ.Gameplay; // 경기 종료 관리자 기능 참조
...
{ // 정상 지점 기능 묶음
    ...
    { // 정상 지점 감지 기능 묶음
        ...
        [SerializeField] private PrototypeMatchController matchController; // 정상 도달 경기 종료 관리자
        ...
        private void Awake() // 정상 지점 시각과 경기 참조 준비
        { // 정상 지점 준비 처리
            if (matchController == null) // 경기 관리자 Inspector 연결 확인
            { // 경기 관리자 자동 검색 처리
                matchController = FindFirstObjectByType<PrototypeMatchController>(); // 현재 Scene의 경기 관리자 조회
            } // 경기 관리자 자동 검색 종료

            if (matchController == null) // 경기 관리자 누락 확인
            { // 경기 관리자 누락 안내 처리
                Debug.LogError("[ProjectJ][Gameplay][COURSE_TOP_MATCH_MISSING] CourseTopTrigger의 PrototypeMatchController 연결을 확인합니다.", this); // 경기 종료 연결 누락 오류 출력
            } // 경기 관리자 누락 안내 처리 종료
        ...
        { // 정상 지점 기본값 준비 처리
            ...
            matchController = FindFirstObjectByType<PrototypeMatchController>(); // 현재 Scene의 경기 관리자 자동 연결
        } // 정상 지점 기본값 준비 종료
        ...
        { // 정상 진입 감지 처리
            ...
            if (matchController != null) // 경기 관리자 연결 확인
            { // 정상 도달 경기 종료 처리
                matchController.TryFinishByCourseTop(respawnController); // 정상 도달자를 기준으로 경기 즉시 종료 시도
            } // 정상 도달 경기 종료 처리 종료
        } // 정상 진입 감지 종료
        ...
        { // 정상 지점 시각 상태 적용 처리
            ...
            { // 재질 속성 블록 지연 생성 처리
                propertyBlock = new MaterialPropertyBlock(); // 재질 속성 블록 생성
            } // 재질 속성 블록 지연 생성 종료
