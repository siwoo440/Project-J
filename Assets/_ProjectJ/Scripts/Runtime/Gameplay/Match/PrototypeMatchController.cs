...
{ // 네임스페이스 범위 시작
    ...
    public sealed class PrototypeMatchController : MonoBehaviour // 단일 플레이 프로토타입 경기 관리자
    { // 클래스 범위 시작
        ...
        [SerializeField] private PlayerRespawnController playerRespawnController; // 플레이어 높이와 부활 정보
        ...
        { // 메서드 범위 시작
            if (playerStateController == null && playerRespawnController != null) // 상태 관리자 자동 검색 조건
            { // 조건 범위 시작
            ...
            { // 조건 범위 시작
                Debug.LogError("[ProjectJ][Gameplay][PROTOTYPE_MATCH_REFERENCE_MISSING] 플레이어 상태와 부활 참조를 확인합니다.", this); // 필수 참조 누락 오류
        ...
        { // 메서드 범위 시작
            ...
            playerRespawnController.StopRespawnForMatchEnd(); // 상태 관리자 기반 입력과 이동 차단
        } // 메서드 범위 종료
