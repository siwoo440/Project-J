...
{ // 설정 복사·검증·JSON·마이그레이션 회귀 테스트 구성
    public sealed class SettingsManagerTests // 설정 기반 단위 테스트 선언
    { // 실제 사용자 디스크 설정을 변경하지 않는 테스트 구성
        ...
        { // 52일차 밝기와 UI 음량 기본값까지 확인
            ...
            Assert.That(settings.Brightness, Is.InRange(0.5f, 1.5f)); // 화면 밝기 유효 범위 검증
            Assert.That(settings.MasterVolume, Is.InRange(0f, 1f)); // 마스터 음량 유효 범위 검증
            Assert.That(settings.UiVolume, Is.InRange(0f, 1f)); // UI 음량 유효 범위 검증
            Assert.That(settings.MouseSensitivity, Is.InRange(0.01f, 2f)); // 마우스 감도 유효 범위 검증
        } // 기본 설정이 현재 버전과 유효 범위를 사용하는지 검증 마무리
        ...
        { // 설정 파일 손상과 UI 잘못된 값 방어 확인
            ...
            Assert.AreEqual(1.5f, settings.Brightness, 0.0001f); // 밝기 최대값 보정 검증
            ...
            Assert.AreEqual(0f, settings.UiVolume, 0.0001f); // UI 음량 최소값 보정 검증
        ...
        { // 52일차 신규 필드를 포함한 직렬화 왕복 확인
            ...
            original.UiVolume = 0.58f; // UI 음량 테스트값 설정
        ...
        } // 설정 JSON 저장과 불러오기 결과가 동일한지 검증 마무리

        [Test] // Unity Test Runner 테스트 지정
        public void JsonSerializerMigratesVersionOneSettings() // 기존 v1 설정이 밝기와 UI 음량 기본값을 보존하며 v2로 이동하는지 검증
        { // 51일차 이전 사용자 설정 파일 호환 확인
            string json = "{\"Version\":1,\"MasterVolume\":0.35,\"MouseSensitivity\":0.45}"; // v1 형태 최소 사용자 설정 JSON 생성
            bool succeeded = SettingsJsonSerializer.TryDeserialize(json, out ProjectUserSettings loaded, out string failureReason); // v1 JSON 마이그레이션 시도
            Assert.IsTrue(succeeded, failureReason); // v1 설정 마이그레이션 성공 검증
            Assert.AreEqual(ProjectUserSettings.CurrentVersion, loaded.Version); // v2 버전 승격 검증
            Assert.AreEqual(0.35f, loaded.MasterVolume, 0.0001f); // 기존 Master 값 보존 검증
            Assert.AreEqual(0.45f, loaded.MouseSensitivity, 0.0001f); // 기존 마우스 감도 보존 검증
            Assert.AreEqual(1f, loaded.Brightness, 0.0001f); // 신규 밝기 100퍼센트 기본값 보강 검증
            Assert.AreEqual(1f, loaded.UiVolume, 0.0001f); // 신규 UI 음량 100퍼센트 기본값 보강 검증
        } // 기존 v1 설정이 밝기와 UI 음량 기본값을 보존하며 v2로 이동하는지 검증 마무리

        [Test] // Unity Test Runner 테스트 지정
        public void JsonSerializerRejectsUnsupportedVersion() // 지원하지 않는 설정 버전을 거부하는지 검증
        { // 미래 버전 설정 파일 복구 기반 확인
