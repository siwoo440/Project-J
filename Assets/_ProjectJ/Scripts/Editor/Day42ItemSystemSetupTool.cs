...
{ // 프로젝트 Editor 기능 묶음
    ...
    { // 42일차 자동 설정 도구 묶음
        ...
        { // 42일차 자동 설정 처리
            ...
            Debug.Log("[ProjectJ][Day42] 28종 데이터, P0 10종 효과, Q/E 슬롯 선택, 우클릭 사용, 상자 가중치와 HUD 안내 구성을 완료했습니다. ITM-014 명칭은 복어 갑옷입니다. Ctrl + S로 Game Scene을 저장합니다.", playerObject); // 자동 설정 완료 로그 출력
        } // 42일차 자동 설정 처리 종료
        ...
        { // 28종 아이템 프로토타입 데이터 생성 처리
            ...
            { // 28종 아이템 프로토타입 묶음
                ...
                new ItemSeed("ITM-011", "Jetpack", "제트팩", "5초 동안 연료를 사용해 상승하고 수평 이동", ItemImplementationPriority.P1, ItemUseType.Duration, ItemEffectType.Jetpack, 6f, 1, 5f, 0f, 0f, 0f, 0f, 0f, 0f, Vector3.one * 0.5f, new Color(0.7f, 0.7f, 0.75f, 1f)), // P1 제트팩
                new ItemSeed("ITM-012", "Hammer", "망치", "6초 동안 기본 밀치기를 강한 근접 공격으로 변경", ItemImplementationPriority.P1, ItemUseType.Duration, ItemEffectType.Hammer, 6f, 1, 6f, 0f, 0f, 0f, 0f, 0f, 0f, Vector3.one * 0.5f, new Color(0.65f, 0.4f, 0.2f, 1f)), // P1 망치
                new ItemSeed("ITM-013", "Bomb", "폭탄", "투척 후 2.5초 뒤 폭발해 주변 대상을 밀어냄", ItemImplementationPriority.P1, ItemUseType.Projectile, ItemEffectType.Bomb, 6f, 1, 2.5f, 0f, 0f, 0f, 5f, 0f, 0f, Vector3.one * 0.5f, new Color(0.15f, 0.15f, 0.15f, 1f)), // P1 폭탄
                new ItemSeed("ITM-014", "PufferArmor", "복어 갑옷", "5초 동안 가까이 접근한 상대를 바깥쪽으로 밀어냄", ItemImplementationPriority.P1, ItemUseType.Duration, ItemEffectType.PufferArmor, 6f, 1, 5f, 0f, 0f, 0f, 0f, 0f, 0f, Vector3.one * 0.5f, new Color(0.95f, 0.65f, 0.2f, 1f)), // P1 복어 갑옷
                new ItemSeed("ITM-015", "InkOctopus", "먹물 문어", "적중한 상대의 화면 중앙을 3.5초 동안 가림", ItemImplementationPriority.P1, ItemUseType.Projectile, ItemEffectType.InkOctopus, 6f, 1, 3.5f, 0f, 0f, 0f, 0f, 0f, 0f, Vector3.one * 0.5f, new Color(0.18f, 0.08f, 0.25f, 1f)), // P1 먹물 문어
                new ItemSeed("ITM-016", "FishingRod", "낚시대", "최대 14m 거리의 상대를 사용자 방향으로 끌어당김", ItemImplementationPriority.P1, ItemUseType.Instant, ItemEffectType.FishingRod, 6f, 1, 0f, 0f, 0f, 14f, 0f, 0f, 0f, Vector3.one * 0.5f, new Color(0.55f, 0.35f, 0.15f, 1f)), // P1 낚시대
                new ItemSeed("ITM-017", "GrapplingHook", "갈고리", "최대 20m 거리의 구조물 방향으로 사용자를 이동", ItemImplementationPriority.P1, ItemUseType.Instant, ItemEffectType.GrapplingHook, 6f, 1, 0f, 0f, 0f, 20f, 0f, 0f, 0f, Vector3.one * 0.5f, new Color(0.45f, 0.5f, 0.55f, 1f)), // P1 갈고리
                new ItemSeed("ITM-018", "SoapBubble", "비눗방울", "대상의 이동과 달리기와 앉기를 제한하고 탈출 입력을 요구", ItemImplementationPriority.P1, ItemUseType.Projectile, ItemEffectType.SoapBubble, 6f, 1, 0f, 0f, 0f, 0f, 0f, 0f, 0f, Vector3.one * 0.5f, new Color(0.7f, 0.9f, 1f, 0.7f)), // P1 비눗방울
                new ItemSeed("ITM-019", "SmokeGrenade", "연막탄", "반경 5m에 6초 동안 시야 방해 구역 생성", ItemImplementationPriority.P1, ItemUseType.Projectile, ItemEffectType.SmokeGrenade, 6f, 1, 6f, 0f, 0f, 0f, 5f, 0f, 0f, Vector3.one * 0.5f, new Color(0.35f, 0.35f, 0.38f, 1f)), // P1 연막탄
                new ItemSeed("ITM-020", "Trampoline", "트램폴린", "사용자 전용 도약 발판을 설치하고 최대 3회 사용", ItemImplementationPriority.P1, ItemUseType.Placement, ItemEffectType.Trampoline, 6f, 1, 0f, 0f, 3f, 0f, 0f, 0f, 0f, new Vector3(0.8f, 0.15f, 0.8f), new Color(1f, 0.3f, 0.55f, 1f)), // P1 트램폴린
                new ItemSeed("ITM-021", "GiantBalloon", "거대 풍선", "6초 동안 계속 상승하며 수평 방향을 조절", ItemImplementationPriority.P1, ItemUseType.Duration, ItemEffectType.GiantBalloon, 6f, 1, 6f, 0f, 0f, 0f, 0f, 0f, 0f, Vector3.one * 0.5f, new Color(1f, 0.45f, 0.65f, 1f)), // P1 거대 풍선
