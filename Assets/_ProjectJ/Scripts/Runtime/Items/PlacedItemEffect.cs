...
{ // 프로젝트 아이템 기능 묶음
    ...
    public sealed class PlacedItemEffect : MonoBehaviour // 바나나 쿠션과 지뢰 설치 효과 선언
    { // 설치 아이템 효과 묶음
        ...
        public void Configure(ItemEffectType newEffectType, Transform newOwnerRoot, Vector3 newForwardDirection, float newForce, float newLifeTime, Vector3 halfExtents, Color visualColor) // 설치 효과와 임시 표시 구성
        { // 설치 효과 구성 처리
        ...
        { // 설치 효과 접촉 처리
            if (other == null || ownerRoot != null && other.transform.IsChildOf(ownerRoot)) // 누락 대상과 설치 사용자 여부 확인
            { // 무시할 접촉 처리
                return; // 설치 효과 적용 생략
            } // 무시할 접촉 처리 종료
