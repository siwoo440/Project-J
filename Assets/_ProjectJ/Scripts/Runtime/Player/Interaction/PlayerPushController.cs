...
{ // 플레이어 밀치기 기능 묶음
    ...
    { // 밀치기 제어 기능 묶음
        ...
        { // 현재 대상 밀치기 처리
            if (CurrentTarget == null || !CurrentTarget.CanReceivePush) // 선택된 대상 존재와 밀치기 가능 상태 확인
            { // 선택 대상 없음 처리
                ClearCurrentTarget(); // 만료된 밀치기 대상 제거
                return; // 밀치기 적용 생략
            } // 선택 대상 없음 처리 종료
        ...
        { // 충돌체 평가 처리
            ...
            if (!target.CanReceivePush) // 부활 보호 또는 비활성 대상 확인
            { // 보호 대상 통과 처리
                return; // 보호 대상을 장애물과 밀치기 대상에서 제외
            } // 보호 대상 통과 처리 종료
            ...
            closestDistance = hitDistance; // 현재 대상 판정 거리 저장
        } // 충돌체 평가 처리 종료
