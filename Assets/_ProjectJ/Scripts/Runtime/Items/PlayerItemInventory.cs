...
{ // 프로젝트 아이템 기능 묶음
    ...
    { // 플레이어 아이템 인벤토리 묶음
        ...
        public bool TryAddItem(ItemDataDefinition itemData, out int placedSlotIndex) // 빈 슬롯 우선 아이템 추가 시도
        { // 아이템 추가 처리
            placedSlotIndex = -1; // 실패 기본 슬롯 번호 설정
            EnsureSlotArray(); // 추가 전 슬롯 배열 보장
        ...
        public bool TryRemoveItemAt(int slotIndex, out ItemDataDefinition removedItem) // 지정 슬롯 아이템 제거 시도
        { // 아이템 제거 처리
            removedItem = null; // 제거 실패 기본 결과 설정
            EnsureSlotArray(); // 제거 전 슬롯 배열 보장
        ...
        { // 인벤토리 초기화 처리
            EnsureSlotArray(); // 초기화 전 슬롯 배열 보장
        ...
        private int CountItems() // 현재 보유 아이템 수 계산
        { // 보유 아이템 수 계산 처리
            EnsureSlotArray(); // 계산 전 슬롯 배열 보장
            int itemCount = 0; // 보유 아이템 수 초기화

            for (int slotIndex = 0; slotIndex < Capacity; slotIndex++) // 두 슬롯 전체 순회
            { // 현재 슬롯 보유 여부 검사 처리
                if (slots[slotIndex] != null) // 현재 슬롯 아이템 존재 여부 확인
                { // 보유 아이템 집계 처리
                    itemCount++; // 보유 아이템 수 증가
                } // 보유 아이템 집계 처리 종료
            } // 현재 슬롯 보유 여부 검사 처리 종료

            return itemCount; // 계산된 보유 아이템 수 반환
        } // 보유 아이템 수 계산 처리 종료
        ...
        private void EnsureSlotArray() // 직렬화 배열을 정확한 두 슬롯으로 보정
        { // 슬롯 배열 보정 처리
            if (slots != null && slots.Length == Capacity) // 올바른 슬롯 배열 여부 확인
            { // 올바른 슬롯 배열 처리
                return; // 기존 배열 유지
            } // 올바른 슬롯 배열 처리 종료
